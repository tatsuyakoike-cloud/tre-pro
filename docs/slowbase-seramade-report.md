...
なので、前提として、SlowBaseはイベント運営会社ではありません。

今回のSeraMadeの話も、  
イベントを運営するという話ではなく、  
**既存顧客の中にある価値を、売上につながる形にできるかを見る取り組み**  
として整理したいです。
...
その中で、ヨガイベントへの協賛という動きが出てきました。

ただ、ここで見たいのはイベントそのものではありません。
...
ここで結果が出れば、  
SlowBaseとしてSeraMadeに対する支援価値を見せやすくなります。
...
この2回で、  
SeraMadeの商品がイベント経由で売上につながるかを見ます。
...
今回のSeraMade支援も、  
イベントを運営するための話ではありません。

SeraMadeという既存顧客の中で、  
SNS運用、EC販売支援、リアルイベントでの反応を見ながら、  
売上につながる導線を作れるかを試す取り組みです。

現状、お酒の支援や撮影営業から大きく案件が広がっているわけではありません。

だからこそ、  
今すでに動いているSeraMadeの事例から、  
SlowBaseが今後目指す方向のヒントを見つけたいです。

SeraMadeの売上を上げる。  
その実績をもとに、餅田農園に対する支援価値を見せる。  
...
*Web 版: `/tre-pro/slowbase-seramade-report/`（GitHub Pages デプロイ時）*
